...
- (void)viewDidLoad {
    ...
    UIView *firstView = [[UIView alloc]initWithFrame:CGRectMake(15, 15, kScreenWidth-30, 44)];
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId", amountTextField.text,@"ExtractingAmount",[NSString stringWithFormat:@"%.2f",self.resultDic],@"ExtraAccountBalance",payTextField.text,@"AlipayAccount", nil];
...
//        NSInteger result = [[responseObj valueForKey:@"result"] intValue];
        NSInteger state = [[[responseObj objectForKey:@"data"] objectForKey:@"state"] integerValue];
        if (state == 1) {
            [self showRightWithTitle:@"提现成功" autoCloseTime:4];
        }else
        {
