...
- (void)viewDidLoad {
    ...
    _xinshoujihao.keyboardType =  UIKeyboardTypeNumberPad;
...
- (IBAction)querenxiugai {
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId", _xinshoujihao.text,@"phone",_yanzhengma.text,@"phoneVerCode",@(1),@"type", nil];
    [BaseHttpTool POST:urlStr params:parameters success:^(id  _Nullable responseObj) {
        if ([responseObj objectForKey:@"result"]) {
            [self showRightWithTitle:@"修改成功" autoCloseTime:2];
            userManager.curUserInfo.userName = _xinshoujihao.text;
