//
//  yiwangongdaipingjiaViewController.m
//  UniversalApp
//
...
@interface yiwangongdaipingjiaViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation yiwangongdaipingjiaViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId",@(_CoutPage),@"currentPage",@(10),@"pageSize",@(6),@"orderState", nil];
        ...
        if (result == 1) {
            if (resultDic != 0) {
                for (int i = 0; i < resultDic.count; i++) {
                    yiwangongdaipingjiaModel *mode = [yiwangongdaipingjiaModel creatRankingTotalModelWith:resultDic[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //使用cell重用机制
    static NSString *cellIdentifier = @"cellIdentifier6";//设置cell重用标示
