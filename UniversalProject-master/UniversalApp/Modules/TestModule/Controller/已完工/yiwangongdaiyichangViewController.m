//
//  yiwangongdaiyichangViewController.m
//  UniversalApp
//
...
@interface yiwangongdaiyichangViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation yiwangongdaiyichangViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId",@(_CoutPage),@"currentPage",@(10),@"pageSize",@(10),@"orderState", nil];
        ...
        if (result == 1) {
            if (resultDic != 0) {
                for (int i = 0; i < resultDic.count; i++) {
                    yiwangongyichangModel *mode = [yiwangongyichangModel creatRankingTotalModelWith:resultDic[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //使用cell重用机制
    static NSString *cellIdentifier = @"cellIdentifier8";//设置cell重用标示
