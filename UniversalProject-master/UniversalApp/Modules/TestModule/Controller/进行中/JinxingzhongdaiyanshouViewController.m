//
//  JinxingzhongdaiyanshouViewController.m
//  UniversalApp
//
...
#import "yiyanshouModel.h"
@interface JinxingzhongdaiyanshouViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation JinxingzhongdaiyanshouViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId",@(_CoutPage),@"currentPage",@(10),@"pageSize",@(4),@"orderState", nil];
        ...
        if (result == 1) {
            if (resultDic != 0) {
                for (int i = 0; i < resultDic.count; i++) {
                    yiyanshouModel *mode = [yiyanshouModel creatRankingTotalModelWith:resultDic[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //使用cell重用机制
    static NSString *cellIdentifier = @"cellIdentifier5";//设置cell重用标示
