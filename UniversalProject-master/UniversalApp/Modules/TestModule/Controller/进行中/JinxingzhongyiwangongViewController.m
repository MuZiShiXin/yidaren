//
//  JinxingzhongyiwangongViewController.m
//  UniversalApp
//
...
@interface JinxingzhongyiwangongViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation JinxingzhongyiwangongViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId",@(_CoutPage),@"currentPage",@(10),@"pageSize",@(3),@"orderState", nil];
    ...
    [BaseHttpTool POST:urlStr params:parameters success:^(id  _Nullable responseObj) {
        ...
        if (result == 1) {
                for (int i = 0; i < resultDic.count; i++) {
                    yiwangongModel *mode = [yiwangongModel creatRankingTotalModelWith:resultDic[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //使用cell重用机制
    static NSString *cellIdentifier = @"cellIdentifier4";//设置cell重用标示
