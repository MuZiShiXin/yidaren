//
//  JinxingzhongdaiquerenViewController.m
//  UniversalApp
//
...
#import "daiqurenModel.h"

@interface JinxingzhongdaiquerenViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation JinxingzhongdaiquerenViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userInfoId",@(_CoutPage),@"currentPage",@(10),@"pageSize",@(1),@"orderState", nil];
        ...
        if (result == 1) {
            if (resultDic != 0) {
                for (int i = 0; i < resultDic.count; i++) {
                    daiqurenModel *mode = [daiqurenModel creatRankingTotalModelWith:resultDic[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //使用cell重用机制
    static NSString *cellIdentifier = @"cellIdentifier2";//设置cell重用标示
