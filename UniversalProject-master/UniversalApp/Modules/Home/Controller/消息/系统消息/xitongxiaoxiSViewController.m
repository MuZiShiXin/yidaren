...
@interface xitongxiaoxiSViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation xitongxiaoxiSViewController
...
- (void)httpRequest
{
    ...
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:@(userManager.curUserInfo.userInfoId),@"userDrId",@(3),@"messagetype", nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    xitongxiaoxitongzhiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    //根据标示去找cell，如果有现成的就用现成的
    if (!cell) {
        //没有现成的cell的时候：
        cell = [[[NSBundle mainBundle]loadNibNamed:@"xitongxiaoxitongzhiTableViewCell" owner:nil options:nil] firstObject];
    }
