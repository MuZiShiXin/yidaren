...
{
//    EnterpriseB = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth/2, 55)];
...
//
//    IndividualB = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth/2, 0, kScreenWidth/2, 55)];
...
{
    UILabel *userPhoneL = [[UILabel alloc]initWithFrame:CGRectMake(30, 5, 40,55)];
    userPhoneL.text = @"帐号";
...
-(void)next:(UIButton *)btn
{
    ...
    if ([enterpriseCipher.text isEqualToString:againenterpriseCipher.text]) {
        ...
        NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:
                                    userPhone.text,@"phone",enterpriseCipher.text,@"password",validationCode.text,@"phoneVerCode",@(3),@"userType", nil];
        ...
        [BaseHttpTool GET:urlStr params:parameters success:^(id  _Nullable responseObj) {
            ...
            if (result == 1) {
                [self.view showRightWithTitle:@"注册成功" autoCloseTime:2];
                if (_block != nil) {
            ...
            {
                btn.userInteractionEnabled = YES;
                [self.view showErrorWithTitle:Message autoCloseTime:2];
            }
...
- (void)getVerificationcCode
{
    ...
    [BaseHttpTool GET:urlStr params:parameters success:^(id  _Nullable responseObj) {
        ...
        {
            NSLog(@"验证码发送失败");
            [self showErrorWithTitle:@"验证码发送成功" autoCloseTime:2];
        }
    } failure:^(NSError * _Nullable error) {
        NSLog(@"loginError:%@",error);
    }];
