...
{
    UILabel *userPhoneL = [[UILabel alloc]initWithFrame:CGRectMake(30, 12, 40,55)];
    userPhoneL.text = @"密码";
    ...
    ConfirmPasswordTF = [[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(userPhoneL.frame)+50, cipherL.mj_origin.y, kScreenWidth-(CGRectGetMaxX(userPhoneL.frame)+50+15), 55)];
    ...
    [determineB setTitle:@"确定" forState:UIControlStateNormal];
...
-(void)determineB
{
    ...
    if (![NewsPasswordTF.text isEqualToString:
        ...
        NSLog(@"密码输入错误");
        [self.view showErrorWithTitle:@"密码输入不一致" autoCloseTime:2];
        return;
    }else if (NewsPasswordTF.text.length < 6)
    {
        [self.view showErrorWithTitle:@"密码不得少于六位" autoCloseTime:2];
        return;
    }else if (ConfirmPasswordTF.text.length < 6)
    {
        [self.view showErrorWithTitle:@"确认密码不得少于六位" autoCloseTime:2];
        return;
    }
    
    
    NSString *urlStr = [NSString stringWithFormat:@"%@/user/xiuGaiMiMa",kPRTURL];
    
    NSDictionary *parameters = [NSDictionary dictionaryWithObjectsAndKeys:self.phoneNumber,@"phone",NewsPasswordTF.text,@"password", nil];
    [BaseHttpTool POST:urlStr params:parameters success:^(id  _Nullable responseObj) {
        ...
        if (result == 1) {
            NSLog(@"更改密码成功");
            [self.view showRightWithTitle:@"修改成功" autoCloseTime:2];
            [self.navigationController popToRootViewControllerAnimated:YES];
        }else
        {
            NSLog(@"更改密码失败");
            [self.view showErrorWithTitle:@"修改密码失败" autoCloseTime:2];
        }
