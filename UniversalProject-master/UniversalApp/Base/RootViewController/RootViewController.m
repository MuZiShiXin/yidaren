...
- (void) setIsShowLiftBack:(BOOL)isShowLiftBack
{
    ...
    if (isShowLiftBack && ( VCCount > 1 || self.navigationController.presentingViewController != nil)) {
        [self addNavigationItemWithImageNames:@[@"yg_dhl_tb1"] isLeft:YES target:self action:@selector(backBtnClicked) tags:nil];
...
- (NSMutableArray<UIButton *> *)addNavigationItemWithTitles:(NSArray *)titles isLeft:(BOOL)isLeft target:(id)target action:(SEL)action tags:(NSArray *)tags
{
    ...
    for (NSString * title in titles) {
        ...
        [btn setTitleColor:[UIColor hx_colorWithHexRGBAString:kBlueColor] forState:UIControlStateNormal];
        btn.tag = [tags[i++] integerValue];
        [btn sizeToFit];
